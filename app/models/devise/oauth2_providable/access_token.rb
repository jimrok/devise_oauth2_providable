class Devise::Oauth2Providable::AccessToken < ActiveRecord::Base
  expires_according_to :access_token_expires_in

  before_validation :restrict_expires_at, :on => :create, :if => :refresh_token
  belongs_to :refresh_token

  attr_accessible :refresh_token, :account_id

  def token_response
    response = {
      :access_token => token,
      :token_type => 'bearer',
      :expires_in => expires_in
    }
    response[:refresh_token] = refresh_token.token if refresh_token
    response
  end


  def find_access_token_by_account_id(account_id)
    Rails.cache.fetch "/access_token_by_account/#{account_id}" do
      token = AccessToken.find_by_account_id account_id
      token.token
    end
  end

  def find_cached_by_token(access_token)
    Rails.cache.fetch "/access_token/#{account_id}" do
      AccessToken.find_by_token(access_token)
    end
  end


  private

  def restrict_expires_at
    self.expires_at = [self.expires_at, refresh_token.expires_at].compact.min
  end
end
